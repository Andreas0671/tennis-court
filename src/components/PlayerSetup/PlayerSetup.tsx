...
interface PlayerSetupProps {
  ...
  roundCount: number;
  readOnly?: boolean;
  hidePlayerListOnMobile?: boolean;
...
export function PlayerSetup(props: PlayerSetupProps) {
  const { players, playerInput, newPlayer, newGender, newStrength, stats, roundCount, readOnly = false, hidePlayerListOnMobile = false } = props;

  return (
    <Card className="rounded-2xl border-0 bg-emerald-50 shadow-sm sm:rounded-3xl">
      ...
      <CardContent className="space-y-4 px-4 pb-4 sm:space-y-5 sm:px-6 sm:pb-6">
        ...
        {players.length > 0 && (
          <div className={`gap-3 md:grid md:grid-cols-2 xl:grid-cols-3 ${hidePlayerListOnMobile ? "hidden" : "grid"}`}>
            {players.map((p) => <PlayerCard key={p.id} player={p} onRemove={readOnly ? undefined : props.onRemove} showStrength={!readOnly} />)}
          </div>
        )}
        {players.length === 0 && (
